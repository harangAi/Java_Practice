...
public class TeamProject {
    ...
    static String[] bookIdx = {"B001", "B002", "B003", "B004", "B005", "B006", "B007", "B008", "B009", "B010"};
    static String[] rentalBookList = {"작별하지 않는다.", "인생은 실전이다."};
    static String[] rentalUserList = {"유재석", "이수근"};

    //현재 저장된 회원 수
    static int count = 3;

    // 회원 정보를 저장할 배열들
    static String[] enoList= {"A009", "A011", "A112"};   // 회원번호
    static String[] nameList = {"유재석", "강호동", "이수근"}; // 회원이름
    static String[] birthList = {"720814", "700611", "750210"};  // 회원생년월일
    ...
    public static int checkEno (String inputId) {
        for (int i = 0; i < enoList.length; i++) {
            if (inputId.equals(enoList[i])) {
                return i;
            }
        }
        return -1;
    ...
    public static void showBookList(int currentPage) {
        ...
        System.out.println("=========================================");
        System.out.println("== | 번호 | 도서명 | 현재 재고 | 도서 번호==");
        System.out.println("=========================================");
        for (int i = startNum; i < endNum; i++) {
            System.out.printf("%d | %s | %2d권 | %s\n", i + 1, bookList[i], bookStock[i], bookIdx[i]);
        }
    }
    ...
    public static int checkBookIdx(String searchBookIdx) {
        ...
        for (int i = 0; i < bookIdx.length; i++) {
            if (searchBookIdx.equals(bookIdx[i])) {
                idx = i;
                break;
            }
        }
    ...
    public static void manageRental() {
        ...
        while (true) {
            ...
            switch (menuStatus) {
                case 1:
                    System.out.println("대여 하기 화면입니다.");
                    rentalBook();
                    break;
                case 7:
                    if(currentPage == 1) {
        ...
        while (true) {
            ...
            if (bookIdx == -1) {
                ...
                if (bookStock[bookIdx] <= 0) {
                    ...
                    rentalUserTemp[rentalUserTemp.length - 1] = nameList[rentalIdx];
    ...
    private static void enoInsert() {
        ...
        enoList = append(enoList, eno);
        nameList = append(nameList, name);
        birthList = append(birthList, age);
        count++;
    ...
    static int searchIndex(String empNumber) {
        int idx;
        for (idx = 0; idx < count; idx++) {
            if (empNumber.equals(enoList[idx])) {
                return idx;
            }
        }
        return -1;
    ...
    static void searchEmpData() {
        ...
        if (index >= 0) {
            System.out.println("====== 검색된 정보 ======");
            System.out.println("# 회원번호: " + enoList[index]);
    ...
    static void processDel(int index) {
        //삭제될 데이터의 이름정보 복사
        String delName = nameList[index];

        enoList = remove(enoList, index);
        nameList = remove(nameList, index);
        birthList = remove(birthList, index);
        count--;
